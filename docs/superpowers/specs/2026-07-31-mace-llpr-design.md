...
3. 三种曲率的结构顺序、原子数、参考值和分量索引对齐；每个 variant 的预测值和残差允许不同，但必须在本路径内满足残差、逐结构力误差和摘要自洽。
...
- 三个 variant 的结构 ID/顺序、原子数、Force 原子/方向键或 reference 不对齐。
- manifest 身份或文件 SHA256 不一致。

发布契约共享的是观测 reference；variant-specific prediction/residual 是合法输入，但每一路径仍独立执行全部 schema、有限性、残差、方差、逐结构聚合和摘要校验。正式评估实现会复用一次模型预测，因此由该流程新生成的三个 variant 仍自然拥有相同 prediction。
