...
1. `direct_test_mad_e0`：直接使用 MAD-r2SCAN test 的 `energy - atomization_energy` 反解 MAD E0，再替换预测总能量中的 MatPES E0；这是 test-informed/oracle 基线。
...
- `direct_test_mad_e0` 直接使用 test 参考标签恢复 MAD E0，并在同一 test 上评估，必须标记为 test-informed/oracle，不能解释为无泄漏泛化结果。
- `model_aware_val_fit` 只使用 validation 拟合和诊断；test 只独立评估，不参与 model-aware 参数拟合。
...
MAD 同时保存 total energy 和 atomization energy。方法 1 直接使用 test：

```text
A_test @ E0_MAD = E_ref_total_test - E_ref_atomization_test
E0_MAD = lstsq(A_test, E_ref_total_test - E_ref_atomization_test, rcond=None)
```
...
### 4.1 direct_test_mad_e0

直接由 test 参考标签恢复 `E0_MAD`，然后对每个 test 成员执行：
...
该方法在校准阶段已经读取 test 的 `energy` 和 `atomization_energy`，因此结果必须记录：

```text
calibration_split = test
uses_test_reference_labels = true
evaluation_role = oracle_baseline
```

它用于量化直接替换 MAD E0 的效果，不得与 `model_aware_val_fit` 一起表述为同等严格的独立 test 泛化实验。
...
禁止：在 `model_aware_val_fit` 中使用 test 标签拟合；按 test 误差选择或调节方法；为每个成员单独拟合；修改 checkpoint 中的 `atomic_energies_fn`；引入 average E0 作为第三种正式方法。
...
- `energy_reference`：纯数值计算组成矩阵、test-informed MAD E0、validation-only model-aware 修正、rank/奇异值/重构诊断。
...
远端顺序：过滤 validation/test -> 8 成员 validation E/F 推理 -> 复用或续跑 test 原始 chunk -> 汇总原始预测 -> 由 test 标签反解 direct MAD E0 -> 仅由 validation 标签和预测拟合 model-aware 参数 -> 对全部 8 个 test 成员应用两套修正 -> 分别生成 Energy 指标/图 -> 生成共享 Force 指标/图 -> 闭包和哈希验证。
...
├── direct_test_mad_e0/{calibration.json,corrected_member_energies.npz,analysis.npz,metrics.json,manifest.json}
...
request identity 绑定 validation/test SHA、过滤索引和 SHA、run manifest SHA、8 个成员路径和 checkpoint SHA、元素表/E0 SHA、domain/精度/chunk 参数、公式版本、`lstsq(rcond=None)`、STD `ddof=1`、GMD pair 规则和代码身份。每个 calibration 记录 89 个元素的 E0_model、E0_MAD、两种 delta、矩阵 shape/rank/奇异值和诊断。`direct_test_mad_e0` manifest 必须写入 `calibration_split=test`、`uses_test_reference_labels=true`、`evaluation_role=oracle_baseline`；`model_aware_val_fit` 必须写入 `calibration_split=validation`、`uses_test_reference_labels=false`、`evaluation_role=calibrated_test`。只有两种 Energy 实验及共享 Force 全部通过后才写总 manifest。

## 7. 统计、绘图和不变量

两种方法分别报告校准诊断、test total Energy 与 Energy/atom MAE/RMSE、Energy/atom uncertainty-residual 相关性和图。方法 1 的 test 指标明确标注 oracle/test-informed，只有方法 2 是 validation-only calibration 后的独立 test 指标。Energy 图使用 corrected ensemble total energy/atom 与 MAD total energy/atom；uncertainty 使用原始 Energy sample STD/atom。Force 结果按逐原子逐 xyz 分量计算，两个方法共享同一套 Force 结果。MAD 不生成 Stress。
...
发布前从修正数组复算验证。图目录分为 `direct_test_mad_e0`、`model_aware_val_fit` 和共享 `shared_force`，不包含 Stress 或成员扫描。
...
本地单元测试使用合成满秩/欠秩矩阵覆盖两种公式、共同平移、Force 不变、非有限值、方法 1 的 test-informed 标记，以及方法 2 禁止读取 test 标签。远端 CPU 小数据闭环覆盖真实 validation/test 小子集、8 成员、两种 Energy、共享 Force、manifest 和 zero-compute reuse。远端全量要求 validation 16,098、test 16,072、两种方法均完成、原始 chunk 未重算/覆盖，并通过图形 QA。

## 9. 完成标准与已确认决策

任务完成必须同时满足：不重训、不改 checkpoint/forward；validation/test 身份和过滤计数正确；方法 1 明确标注 test-informed/oracle；方法 2 严格保持 validation 拟合/test 评估隔离；两种方法对全部 8 成员都有独立校准、Energy 结果、指标、图和 manifest；共享 Force 完整且哈希引用一致；UQ 不变量、全量闭包、zero-compute reuse 和远端 QA 均通过；Git 不含大规模结果。

已确认：方法 1 直接使用 MAD-r2SCAN test 的 `energy - atomization_energy` 恢复 MAD E0，不使用 validation，并作为 test-informed/oracle 基线；方法 2 使用独立 MAD-r2SCAN validation 和 8 成员预测均值拟合，同一修正应用所有成员后在 test 评估；两种方法都必须实验；MAD 只处理 Energy/Force；原始推理不可变；本地只开发，远端执行；不直接复用 carnet 代码。
